...
export default function CompareModal({ open, onClose, persons, data }) {
  ...
  const filteredHeroes = HEROES.filter((h) =>
    h.nameFr.toLowerCase().includes(heroFilter.toLowerCase()) ||
    ...
    <div style={{
      ...
      <div onClick={(e) => e.stopPropagation()} style={{
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
          <h2 style={{ margin: 0, fontSize: 20, color: "var(--text)", fontFamily: "var(--font-display)", textTransform: "uppercase" }}>Comparaison</h2>
        ...
        <div style={{ marginBottom: 20 }}>
          ...
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
            {personNames.map((name) => (
              <button key={name} onClick={() => togglePerson(name)} style={{
        ...
        {selected.length >= 2 && (
          <>
            <input type="text" placeholder="🔍 Filtrer les héros..."
                  ...
                  <tr style={{ borderBottom: "2px solid var(--border-light)" }}>
                    ...
                    {selected.map((name) => (
                      <th key={name} style={{ ...thStyle, minWidth: 160 }}>
                        <span style={{ color: "var(--red-light)", fontFamily: "var(--font-display)", textTransform: "uppercase" }}>{name}</span>
                      </th>
                    ))}
                  </tr>
                </thead>
                        ...
                        <td style={{ padding: "10px 8px" }}>
                          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                            <div style={{
                              ...
                              <img src={getHeroImagePath(hero.slug)} alt=""
                                style={{ width: "100%", height: "100%", objectFit: "cover" }}
                                onError={(e) => {
                                  e.target.style.display = "none";
                                  e.target.parentElement.innerHTML = `<span style="font-size:12px;font-weight:700;color:#fff">${getInitials(hero.nameFr)}</span>`;
                                }} />
                            </div>
                            <span style={{ fontWeight: 700, fontFamily: "var(--font-display)", textTransform: "uppercase", fontSize: 11 }}>
                              {hero.nameFr}
                            </span>
                          </div>
                        </td>
                          ...
                          return (
                            <td key={name} style={{ padding: "10px 8px", verticalAlign: "top" }}>
                              ...
                              <div style={{ fontSize: 11, color: "var(--text-muted)", marginBottom: 4 }}>
                                ...
                                {d.prestige !== "white" && (
                                  <span style={{ color: prest.color, marginLeft: 6, fontWeight: 700, fontSize: 10 }}>{prest.label}</span>
                                )}
                              </div>
                              {d.comments.length > 0 && (
                                <div>
                                  <button onClick={() => toggleComments(commentKey)} style={{
